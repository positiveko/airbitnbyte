...
const NavComponent = styled.header`
  ${({ state }) =>
    state === 'main'
      ? css`
          ...
          box-shadow: ${({ scrollYdata }) =>
            scrollYdata && `0 -1px 5px 1px rgba(0, 0, 0, 0.3)`};
        `
      : css`
  ...
  z-index: 110;
  transition: 0.25s;
  background-color: ${({ scrollYdata, state }) =>
    !scrollYdata && state === 'main' ? 'none' : 'white'};
  ...
  ${({ state }) =>
    state === 'main'
      ? css`
          ...
          opacity: ${({ scrollYdata, foldNav }) =>
            !scrollYdata || foldNav ? 1 : 0};
        `
      : css`
          transform: ${({ foldNav }) =>
            foldNav ? `translate(0, 0)` : `translate(0, -100px) scaleX( 0.5 )`};
