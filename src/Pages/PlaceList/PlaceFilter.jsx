...
const PlaceFilterPlaceList = ({
  ...
  useEffect(() => {
    const closeModal = ({ target }) => {
      if (
        Object.keys(modalState).some((el) => modalState[el]) &&
  ...
  const openModal = (e) => {
    ...
    Object.keys(modalState).forEach((filter) => {
      filter === dataset.filter &&
        setModalState({ ...modalState, [dataset.filter]: true });
    });
  };
  ...
  return (
    <FilterComponent ref={modal}>
      <div
        className='filter HouseTypeFilter'
        data-filter='houseFilter'
        onClick={openModal}>
        <span>숙소유형</span>
      ...
      <div
        className='filter moneyFilter'
        data-filter='moneyFilter'
        onClick={openModal}>
        <span>요금</span>
        <FilterModal state={modalState.moneyFilter}>요금</FilterModal>
      </div>
      <div
        className='filter otherFilter'
        data-filter='otherFilter'
        onClick={openModal}>
        <span>필터 추가히기</span>
...
const FILTER_HOUSE_TYPE = [
  ...
  },
  {
    type: 3,
    title: '호텔 객실',
    content: '부티크 호텔, 호스텔 등의 개인실이나 다인실을 이용합니다.',
  },
  {
    type: 4,
    title: '다인실',
