...
const PlaceList = (props) => {
  ...
  useEffect(() => dispatch(setNavState('list')), []);
  ...
  const getPlaceList = async () => {
    const pageNation = `${
      currentPage > 1 ? `offset=${currentPage}&limit=10` : `limit=10`
    }`;
  ...
  const toQueryString = (data, type) => {
    if (type === undefined) {
      ...
      Object.entries(data).forEach(
        (filterData) => filterData[1] && result.push(filterData.join('='))
      );
  ...
  return (
    <PlaceListComponent>
          ...
          placeList.map((itemData, index) => {
            return (
              <PlaceListItem
                listItem={itemData}
                key={index}
                onMouseOver={({ latitude, longitude }) =>
                  sethoverLocation({ latitude, longitude })
                }></PlaceListItem>
            );
          })
        ) : (
      ...
      {placeList && (
        <PlaceListMap placeList={placeList} hoverLocation={hoverLocation} />
      )}
      <LoadingSpinner state={lodingSpinner}>
        <img
          className='spinner'
          src='images/airBnB_pink.png'
          alt='스피너 이미지'></img>
      </LoadingSpinner>
    </PlaceListComponent>
  );
